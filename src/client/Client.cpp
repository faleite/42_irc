...
Client::Client()
    : _clientSocket(-1), _ip(""), _port(0), _name(""), 
      _nickName(""), _isAuthenticated(false), _isOperator(false) {
  // std::cout << "Default Client Constructor Called" << std::endl; // Tolk about it
}

Client::Client(int clientSoket, std::string ip, int port)
    : _clientSocket(clientSoket), _ip(ip), _port(port), _name(""), 
      _nickName(""), _isAuthenticated(false), _isOperator(false) {
  // std::cout << "Client Socket Constructor Connected" << std::endl; // Tolk about it
}

Client::Client(const Client &copyObj)
{
  *this = copyObj;
}
...
Client::~Client() {} // { std::cout << "Client Destroyed" << std::endl; }; // Tolk about it
