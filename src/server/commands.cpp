...
void Server::quit(Client &client, const std::string &cmd, const std::vector<std::string> &param)
{
	...
	std::cout << "Client disconnected with QUIT command :fd: " << client.getSocket() << std::endl;
...
void Server::nick(Client &client, const std::string &cmd, const std::vector<std::string> &param)
{
  if (param.empty())
  {
	...
	client.getMessage(":" + client.getNickName() + "!" + client.getName() + "@localhost " + cmd + " " + param[0]);
